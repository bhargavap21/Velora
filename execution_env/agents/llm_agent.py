...
Unlike the PPO agent (which decides a per-slice participation multiplier relative to
the volume curve), the LLM agent sets a coarser per-episode execution *schedule* (e.g.
"front-load 60% in the first third," "match the volume curve," "hold back if early
slices show adverse price movement") and that schedule is then played out slice-by-slice
against the same ExecutionEnv. This keeps the LLM call budget to ~1 call per episode
...
# ---------------------------------------------------------------------------
# ExecutionEnv.step() takes a *participation multiplier* relative to the volume-curve-
# implied fair share of remaining inventory (1.0 == "trade exactly at the VWAP-implied
# rate") -- not an absolute quantity. All primitives below work in absolute "fraction of
# total" space and then call _qty_to_participation() to convert, so the math inside each
# primitive stays intuitive.


def _qty_to_participation(target_weights: np.ndarray, volume_curve: np.ndarray) -> list[float]:
    """Convert a per-slice absolute-quantity-weight distribution (e.g. front-loaded) into
    a sequence of participation multipliers that reproduces it under ExecutionEnv's
    volume-curve-relative action semantics.

    The last slice is always forced to 1.0, which under ExecutionEnv's semantics always
    consumes exactly whatever inventory remains, so floating-point drift across
    primitives never leaves the order partially unfilled.
    """
    ...
    target_fracs_of_total = target_weights / total
    multipliers: list[float] = []
    remaining_frac = 1.0
    for i in range(n - 1):
        remaining_curve_weight = float(volume_curve[i:].sum())
        fair_share_frac = volume_curve[i] / remaining_curve_weight if remaining_curve_weight > 0 else 0.0
        if remaining_frac <= 1e-9 or fair_share_frac <= 0:
            multipliers.append(0.0)
            continue
        desired_frac_of_remaining = min(target_fracs_of_total[i] / remaining_frac, 1.0)
        multipliers.append(float(np.clip(desired_frac_of_remaining / fair_share_frac, 0.0, 2.0)))
        remaining_frac = max(remaining_frac - target_fracs_of_total[i], 0.0)
    multipliers.append(1.0)
    return multipliers


def front_load(pct: float, n_slices: int, volume_curve: np.ndarray) -> list[float]:
    """Execute pct of total inventory in the first third of slices, rest evenly.

    Example: front_load(0.6, 26, vc) puts 60% in slices 0-8, 40% in slices 9-25.
    """
    pct = float(np.clip(pct, 0.0, 1.0))
    early = max(1, n_slices // 3)
    late = n_slices - early
    weights = np.array([pct / early] * early + [(1.0 - pct) / max(late, 1)] * late)
    return _qty_to_participation(weights, volume_curve)


def back_load(pct: float, n_slices: int, volume_curve: np.ndarray) -> list[float]:
    """Execute pct of total inventory in the final third of slices, rest evenly.

    Example: back_load(0.6, 26, vc) puts 40% in slices 0-17, 60% in slices 18-25.
    """
    pct = float(np.clip(pct, 0.0, 1.0))
...
def follow_volume_curve(n_slices: int, volume_curve: np.ndarray) -> list[float]:
    """Weight each slice proportionally to the day's actual intraday volume curve.
    ...
    Under ExecutionEnv's volume-curve-relative action semantics this collapses to a
    constant participation of 1.0 -- "trade exactly the VWAP-implied rate every slice."
    """
    return _qty_to_participation(np.asarray(volume_curve, dtype=float), volume_curve)


def twap(n_slices: int, volume_curve: np.ndarray) -> list[float]:
...
def build_observation(
    ...
    pause modifier) -- we expand it to per-slice participation multipliers in
...
def schedule_from_response(response_text: str, n_slices: int, volume_curve: np.ndarray) -> tuple[list[float], dict]:
    """Parse Claude's JSON response into a per-slice participation-multiplier list.

    Returns (multipliers, parsed_json). Raises ValueError on invalid primitive names
    or missing required fields so the caller can surface a useful error.
    """
    ...
    if primitive == "front_load":
        multipliers = front_load(pct, n_slices, volume_curve)
    elif primitive == "back_load":
        multipliers = back_load(pct, n_slices, volume_curve)
    elif primitive == "follow_volume_curve":
        multipliers = follow_volume_curve(n_slices, volume_curve)
    elif primitive == "twap":
        multipliers = twap(n_slices, volume_curve)
    else:
        raise ValueError(
...
def propose_schedule(
    ...
    Returns (multipliers, parsed_json, raw_response_text). The raw text is kept for
    logging/debugging; callers should use multipliers for execution.
...
def run_episode(env, *, verbose: bool = False) -> dict:
    ...
    skipped (participation = 0.0). The final slice's multiplier is always 1.0 (from
    _qty_to_participation), so any paused inventory is executed at close.
    ...
    last_exec_price: float | None = None  # last slice where multiplier > 0
    ...
    for i in range(n_slices):
        ...
        # Pause modifier: skip slice if price has risen adversely since last fill. Never
        # applies to the final slice -- that one is always forced to fully fill the
        # order (see _qty_to_participation), and letting the pause override it would
        # leave inventory unfilled, which the reward penalizes far more than any slippage.
